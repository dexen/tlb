...
function wiki_post_title_to_html(array $rcd) : string
{
	return '<h1>' .H(wiki_camel_to_spaced($rcd['_url_slug'])) .'</h1>';
}

function wiki_text_to_linked(string $str) : string
{
...
{
	$str = $rcd['body'];
	return '<p>' .implode('</p><p>', array_map('wiki_text_to_linked', array_map('H', explode("\n\n", $str)))) .'</p>';
}
